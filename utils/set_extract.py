...
#     speed_up_dict['mix'] = {}
#     for p in parts_name:
#         ipc_get_func = multi_stats_lastn_factory(t.cache_set_targets,[f"cpu{i}.ipc" for i in range(nsamples)],last_n=20)
...
def extract_set_trace(stats_dir,n_id = 2,last_nsamples = 40):
    n_l3_sets = 4096
    set_access_dict = {}
...
def extact_all_set_trace(stats_top_dir):
    workpair_name = os.listdir(stats_top_dir) #like mcf-omnetpp
    for wp in workpair_name:
        policy_dir = os.path.join(stats_top_dir,wp)
        policy_names = os.listdir(policy_dir)
        waymask_names = list(filter(lambda x: x.startswith('0'), policy_names))
        for wayname in waymask_names:
            stats_dir = os.path.join(policy_dir,wayname)
            extract_set_trace(stats_dir)
...
if __name__ == '__main__':
    ...
    # extact_all_set_trace("/nfs/home/zhangchuanqi/lvna/5g/ff-reshape/log/40x1M/")
