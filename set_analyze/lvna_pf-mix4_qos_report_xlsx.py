...
xlsx_drop_res = [
    re.compile(r'basePart'),
]
force_update_res = [
    # re.compile(r'cppf.*UnusePF'),
]

xlsx_postfix = 'LabelPFCtrlLv'
...
def analyze_one_workload_dict(base_dir,workload_names,ncore=4):
    ...
    baseline_cpuipcs = [ s_dicts['basePart'][f'cpu{c}.ipc'][0] for c in range(ncore) ]
    s_dicts.pop('basePart')
    ...
    for k in sorted(s_dicts.keys()):
        ...
        pd_dict[f'{k}_hp0'] = pd_dict[f'{k}_relperf0']
        pd_dict[f'{k}_besum'] = speedup_sum - pd_dict[f'{k}_relperf0'] - (ncore-1)
...
def run_one_conf_one_bench(select_json:str,bench_choice:str):
    ...
    base_dir = f'/nfs/home/zhangchuanqi/lvna/for_xs/catlog/lvnapf-mix-test/mix{ncore}-{wm_len}-qos-bench{bench_choice}'
    ...
    for i,work in enumerate(worksname):
        ...
        policy_names = [ k.rsplit('_',1)[0] for k in speedupsum_keys ]

        speedupsum_dict = { 'workload': work }
        hpbe_dict = { 'workload': work }
        for pn in policy_names:
            ...
            for dre in xlsx_drop_res:
                if dre.search(pn):
                    ...
                    break
            if drop_flag:
                pd_dict.pop(f'{pn}_speedupsum')
                pd_dict.pop(f'{pn}_hp0')
                pd_dict.pop(f'{pn}_besum')
            else:
                speedupsum_dict[pn] = pd_dict.pop(f'{pn}_speedupsum')
                hpbe_dict[f'{pn}_hp0'] = pd_dict.pop(f'{pn}_hp0')
                hpbe_dict[f'{pn}_besum'] = pd_dict.pop(f'{pn}_besum')

        speedup_sum_list.append(speedupsum_dict)
        hpbe_list.append(hpbe_dict)

        pd_dict_list.append(pd_dict)

    xlsx_path = f'set_analyze/{test_prefix}xls/qos-mix{ncore}-{wm_len}-{xlsx_postfix}.xlsx'
    ...
    df = pd.DataFrame(hpbe_list)
    df.to_excel(writer, sheet_name=f'{bench_choice}-hp0-besum',
                index=False, float_format="%.4f")
