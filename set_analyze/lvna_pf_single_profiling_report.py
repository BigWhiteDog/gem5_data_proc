...
def draw_one_workload_cat_ipc(ax,s_dicts,workload_names):

    fullcache_pf_ipc = s_dicts['nopart']['cpu.ipc'][0] 
    pf_cat_normal_ipc = [s_dicts[f'PrefetchSingleCorePolicy-{w}']['cpu.ipc'][0]
                         /fullcache_pf_ipc for w in range(1,max_assoc+1)]
    x_labels = list(range(1,max_assoc+1))
    ax.plot(x_labels,pf_cat_normal_ipc,marker='o',markersize=8,label='pf-cat')
    # ax.plot(x_labels,normal_nopf_ipc,marker='D',markersize=8,label='nopf')
    ax.set_ylabel('Norm IPC')
    ax.axhline(y=1,ls='--',color='tab:brown',linewidth=2)
