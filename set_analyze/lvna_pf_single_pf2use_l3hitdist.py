...
def draw_one_workload_l1_hit_age(ax,s_dicts,workload_name,full,pos:tuple):
    l3_hit_ages = s_dicts['l3_hit_ages']
    #calculate id = 0 hit age
    demand_hit_ages = l3_hit_ages[0]
    pf_hit_ages = list(itertools.chain(*l3_hit_ages[1:]))
    total_hit_ages = list(itertools.chain(demand_hit_ages,pf_hit_ages))

    age_lists = [demand_hit_ages,pf_hit_ages,total_hit_ages]
    label_lists = ['demand','pf','total']
    ...
    for i in range(3):
        ...
        ax.plot(bin_centers,hitage_cdf,label=f'{label_lists[i]}',color=contrasting_orange[i], marker='o')
...
def draw_db_by_func(base_dir,n_rows,worksname_waydict,draw_one_func,fig_name,n_cols=6,
    ...
    for i,work in enumerate(worksname_waydict):
        ...
        if dict_updated:
            ...
            all_access_query = 'SELECT REQID,PREFAGE FROM L3XsTrace WHERE HITMISSINS = 0 ORDER BY ID ASC;'
            f = cur.execute(all_access_query)

            for reqid,prefage in f:
                reqid = int(reqid)
                prefage = int(prefage)
                l3_hit_ages[reqid].append(prefage)
...
def run_one_conf(select_json:str):
    ...
    draw_db_by_func(base_dir,n_rows,interested_works,
                    n_cols=n_cols,
        draw_one_func=draw_one_workload_l1_hit_age,fig_name=os.path.join(pic_dir_path,'pf2use_l3hit_dist.png'))
