...
def analyze_one_workload_dict(base_dir,workload_names,ncore=2):
    ...
    for part in partsname:
        ...
        if re.search(r'fullgrow(\d+)in16',dkey):
            ...
            continue
        last_nsamples=1
        if not os.path.exists(os.path.join(new_base,f'{last_nsamples}period.json')):
    ...
    for k in sorted(s_dicts.keys()):
        ...
        if get_policy_res:
            ...
            new_key = policy
            if policy in ['OneLessGrowTarget','RealOneLess']:
            # if policy in ['OneLessGrowTarget','NoGrowRealOneLess']:
                continue
            if get_train_res:
    ...
    for tstep in tstep_list:
        ...
        for perf in target_perf_list:
            ...
            for w0 in full_grow_dict:
                ...
                # find_grow_target[w0] = min(target_max_s1.keys())
                print(f'w0:{w0} find_{maxbuckets}_grow_target:{find_grow_target[w0]}')
