...
def run_one_conf(select_json:str):
    ...
    way_dict = use_conf["cache_work_95perfways"]
    max_assoc = use_conf["max_assoc"]

    #generate work lists
    work_lists = []

    for hp0 in need_work_names:
        hp0_way = way_dict[hp0]
        for hp1 in need_work_names:
            if hp1 == hp0:
                continue
            hp1_way = way_dict[hp1]
            if hp0_way+hp1_way >= max_assoc:
                continue
            #now sample others
            one_sample_num = 2
            iter_num = 2
            ...
            for _ in range(iter_num):
                ...
                iter_work_name_set.remove(hp0)
                iter_work_name_set.remove(hp1)
                #handle remain
    ...
    new_json_name = old_json_name.replace("conf","benchs_2hp_4")
    new_json_path = os.path.join(json_dir_path,new_json_name)

    final_dict = {}
    final_dict['4bench'] = work_lists
