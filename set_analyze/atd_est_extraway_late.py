    ...
    def handle_look_ahead_hit(self, hitpos):
        ...
        if hitpos >= self.la_est_need:
            ...
            if self.start_est:
                ...
                #if haven't start est, then just update est need
                self.la_est_need = self.la_update_est_need
...
def report_csvsum_atd_lookahead(usepd,s_dicts,workload_name,full_ass):
    ...
    max_stamp = max(hitpos_set_records,key=lambda x: x[-1][1] if(len(x)) else 0)[-1][1]

    max_stamp_10 = max_stamp * 0.1

    la_states = [SetLookAheadState(s) for s in range(all_set)]

    access_before_est = 0
    hit_before_est = 0
    for la,hitpos_rec in zip(la_states,hitpos_set_records):
        for hitpos,stamp in hitpos_rec:
            if stamp > max_stamp_10:
                la.start_est = True
            else:
                access_before_est += 1
                hit_before_est += 1 if hitpos >= 0 else 0

            if hitpos >=0:
                #hit
                la.handle_look_ahead_hit(hitpos)
            else:
                #miss
                la.handle_look_ahead_miss()

    new_dict = {}

    new_dict['sum_hits'] = sum(l.hit_cnts for l in la_states)
    new_dict['sum_access'] = sum(l.access_cnts for l in la_states)
    new_dict['sum_misses'] = new_dict['sum_access'] - new_dict['sum_hits']
    new_dict['miss_rate'] = new_dict['sum_misses'] / new_dict['sum_access']
    new_dict['sum_la_extra_miss'] = sum(l.la_extra_miss for l in la_states)
    new_dict['sum_la_extra_miss_rate'] = new_dict['sum_la_extra_miss'] / new_dict['sum_access']
    new_dict['sum_la_extra_miss_ingrow'] = sum(l.la_extra_miss_ingrowprogress for l in la_states)
    new_dict['sum_la_extra_miss_ingrow_rate'] = new_dict['sum_la_extra_miss_ingrow'] / new_dict['sum_access']
    new_dict['last_delta_stamp'] = max_stamp
    new_dict['last_delta_stamp_10'] = max_stamp_10
    new_dict['access_before_est'] = access_before_est
    new_dict['hit_before_est'] = hit_before_est
...
class SetLRUStates:
    def __init__(self, set_id, full_ass):
        ...
        self.hitpos_stamp_list = []
    
    def record_hitpos_stamp(self, hitpos, stamp):
        self.hitpos_stamp_list.append((hitpos,stamp))
    ...
    def newcome(self, tag, stamp):
        ...
        if len(res) > 0:
            ...
            self.record_hitpos_stamp(hit_pos, stamp)
            #modify lru
            ls.remove(get_pair)
            ls.add([stamp,tag])
        else:
            ...
            if len(ls) >= self.full_ass:
                ls.pop(0)
            ls.add([stamp,tag])
            #record miss pos stamp
            self.record_hitpos_stamp(-1, stamp)
...
def analyze_workload_len_est(work_stats_dict,work,work_dir,full_ass):
    ...
    s_dicts['hitpos_stamp_record'] = [l.hitpos_stamp_list for l in lru_states]
...
if __name__ == '__main__':
    ...
    drawF_picf_jsonf_csvF_csvsumf = [
        (draw_growlist,'atd-nothing{}.png','atd_est_hitpos_stamp_record_{}.json',report_csvsum_atd_lookahead,'atd_est_late_{}.csv'),
