...
def draw_ipc_access_plot(ax,s_dicts,workload_name,full,pos:tuple):
    ...
    work_pfsrc_property = use_conf['work_pfsrc_property'][workload_name]
    for pfs in range(pf_src_num):
        if pfs not in work_pfsrc_property:
            work_pfsrc_property[pfs] = {}
        work_pfsrc_property[pfs]['l3AccessRate'] = s_dicts['l3.accessPFSrc'][pfs]/l3Access_all
        work_pfsrc_property[pfs]['nipc'] = nipcs[pfs]
...
def run_one_conf(select_json:str):
    ...
    use_conf['work_pfsrc_property'] = {}
    for w in worksname:
        use_conf['work_pfsrc_property'][w] = {}
    ...
    draw_db_by_func(base_dir,n_rows,interested_works,
                    n_cols=n_cols,
        draw_one_func=draw_ipc_access_issue_bar,fig_name=os.path.join(pic_dir_path,f'pf-{test_way}way-bypassPfSrc.png'))
    draw_db_by_func(base_dir,n_rows,interested_works,
                    n_cols=n_cols,
        draw_one_func=draw_ipc_access_plot,fig_name=os.path.join(pic_dir_path,f'pf-{test_way}way-bypassPfSrc-nipc.png'))
