...
def draw_one_workload_way_need_withmr(ax,s_dicts,workload_name,full_ass,pos:tuple):
    label_s = ['min_ways_no_extra_miss','min_ways_1_extra_miss', 'min_ways_2_extra_miss','full_ways_miss_rate']
    extra0_list = s_dicts[label_s[0]]
    extra1_list = s_dicts[label_s[1]]
    extra2_list = s_dicts[label_s[2]]
    full_mr_list = s_dicts[label_s[3]]
    sorted_setlist = sorted(zip(extra0_list,full_mr_list,extra1_list,extra2_list))
    s_extra0_list,s_full_mr_list,s_extra1_list,s_extra2_list = zip(*sorted_setlist)
    x_val = np.arange(all_set)
    full_ass_vals = np.full(all_set,full_ass)
    ...
    ax2 = ax.twinx()
    ax2.plot(s_full_mr_list, label='full ways miss rate', color = contrasting_orange[9],linewidth=2)
    ax2.set_ylabel('miss rate of the set')
    ax2.set_ylim(0, 1)
