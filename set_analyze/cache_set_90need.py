...
def draw_one_workload_way_need(ax,s_dicts,workload_name,full_ass,pos:tuple):
    ...
    ax.plot(s_extra0_list, label='ways with same hit cnts of the set', color = extra0_list_color,linewidth=2)
    ax.fill_between(x_val, full_ass_vals, s_extra0_list, color = extra0_list_color, alpha=alpha_set)
    ax.plot(full_ass_vals,label='95% perf CAT set ways', color = contrasting_orange[4],linewidth=2)
    ax.set_ylabel('needed ways')
    ax.set_ylim(0, 8.1)
    ax.yaxis.set_major_locator(ticker.MultipleLocator(1))
    ax.set_xlabel('set index (sorted by min needed ways)')
    ax.set_title(f'{workload_name}')
    if pos == (0,0):
        ax.legend(shadow=0, fontsize = 15, bbox_to_anchor=(-0.01,1.35), loc = 'upper left',  \
...
def draw_one_workload_way_need_withmc(ax,s_dicts,workload_name,full_ass,pos:tuple):
    label_s = ['min_ways_no_extra_miss','ways_miss_cnt']
    extra0_list = s_dicts[label_s[0]]
    full_mc_list = s_dicts[label_s[1]][full_ass]
    sorted_setlist = sorted(zip(extra0_list,full_mc_list))
    s_extra0_list,s_full_mc_list = zip(*sorted_setlist)
    x_val = np.arange(all_set)
    full_ass_vals = np.full(all_set,full_ass)
    ...
    ax2 = ax.twinx()
    ax2.plot(s_full_mc_list, label='full ways miss rate', color = contrasting_orange[9],linewidth=1)
    ax2.set_ylabel('miss count of the set')
    sorted_onlymc = sorted(s_full_mc_list)
    tail995_mc = sorted_onlymc[int(len(sorted_onlymc)*0.995)]
    ax2.set_ylim(0, tail995_mc)
    # ax2.set_ylim(0, 1)
...
def draw_db_by_func(base_dir,n_rows,worksname_waydict,draw_one_func,fig_name,n_cols=6):
    ...
    fig.set_size_inches(n_cols*6, 4.5*n_rows+3)
...
if __name__ == '__main__':
    ...
    cache_work_95perfmax8ways = use_conf['cache_work_95perfmax8ways']
    ...
    draw_db_by_func(base_dir,n_rows,cache_work_95perfmax8ways,
        draw_one_func=draw_one_workload_way_need,fig_name=os.path.join(pic_dir_path,'way_need_95perfmax8_dis.png'))
